...
### Tidyverse/GGPLOT2 scatterplots
...
Adding a line of best fit is  easy, but it takes a bit of getting used to
...
# Create dummy data. You will always have your own data
data <- data.frame(
...
```r
# Add with linear trend line/line of best fit
ggplot(data, aes(x=my_x, y=my_y)) +
  geom_point() +
...
You can use the plotly library to make your plots interactive (really useful, try zooming in or clicking on a few points)
...
```r
hist(HousesNY$Price, xlab="Price (USD)")
```
